import { useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import axios from 'axios'
import Swal from 'sweetalert2'

const LOGIN_URL = 'http://192.3.193.173/api/auth/login'

const LoginForm = () => {
  const [formData, setFormData] = useState({
    username: '',
    password: '',
  })

  const [errors, setErrors] = useState({
    username: '',
    password: '',
  })

  const [loading, setLoading] = useState(false) 
  const navigate = useNavigate()

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFormData((prevData) => ({
      ...prevData,
      [e.target.name]: e.target.value,
    }))

    setErrors((prevErrors) => ({
      ...prevErrors,
      [e.target.name]: '',
    }))
  }

  const validate = () => {
    let valid = true
    const newErrors = { username: '', password: '' }

    if (!formData.username.trim()) {
      newErrors.username = 'Username tidak boleh kosong'
      valid = false
    }

    if (!formData.password) {
      newErrors.password = 'Password tidak boleh kosong'
      valid = false
    }

    setErrors(newErrors)
    return valid
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()

    if (!validate()) {
      return
    }

    setLoading(true) // Aktifkan spinner

    try {
      const response = await axios.post(
        LOGIN_URL,
        {
          username: formData.username,
          password: formData.password,
        },
        {
          headers: { 'Content-Type': 'application/json' },
        },
      )

      console.log('Login Berhasil:', response.data)

      const { user, access_token } = response.data.data
      localStorage.setItem('user', JSON.stringify(user))
      localStorage.setItem('token', access_token)

      Swal.fire({
        toast: true,
        position: 'top-end',
        icon: 'success',
        title: 'Login berhasil!',
        showConfirmButton: false,
        timer: 1500,
        timerProgressBar: true,
      })

      setTimeout(() => {
        navigate('/dashboard')
      }, 1500)
    } catch (error: unknown) {
      console.error('Login error:', error)

      Swal.fire({
        toast: true,
        position: 'top-end',
        icon: 'error',
        title: 'Login gagal! Periksa kembali kredensial Anda.',
        showConfirmButton: false,
        timer: 1500,
        timerProgressBar: true,
      })
    } finally {
      setLoading(false) // Matikan spinner setelah request selesai
    }
  }

  return (
    <>
      <form onSubmit={handleSubmit} className="w-md p-4 rounded-lg ml-14 mt-6">
        <div className="flex flex-col mb-4">
          <label
            className="text-[#474747] mb-2 font-medium text-xl"
            htmlFor="username"
          >
            Username
          </label>
          <input
            type="text"
            id="username"
            name="username"
            value={formData.username}
            onChange={handleChange}
            className="w-full p-2 pl-10 text-lg text-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 bg-white shadow-md"
          />
          {errors.username && (
            <p className="text-red-500 text-sm mt-1">{errors.username}</p>
          )}
        </div>

        <div className="flex flex-col mb-4">
          <label
            className="text-[#474747] mb-2 font-medium text-xl"
            htmlFor="password"
          >
            Password
          </label>
          <input
            type="password"
            id="password"
            name="password"
            value={formData.password}
            onChange={handleChange}
            className="w-full p-2 pl-10 text-lg text-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 bg-white shadow-md"
          />
          {errors.password && (
            <p className="text-red-500 text-sm mt-1">{errors.password}</p>
          )}
        </div>

        <button
          type="submit"
          className={`w-full p-3 bg-[#E8886B] text-white rounded-lg hover:bg-[#db9079] focus:outline-none focus:ring-2 focus:ring-gray-600 flex items-center justify-center ${
            loading ? 'opacity-75 cursor-not-allowed' : ''
          }`}
          disabled={loading}
        >
          {loading ? (
            <svg
              aria-hidden="true"
              className="w-6 h-6 text-white animate-spin"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="#ffffff"
              />
            </svg>
          ) : (
            'Login'
          )}
        </button>

        <Link to="/register">
          <p className="text-center mt-4 hover:underline">Create Account</p>
        </Link>
      </form>
    </>
  )
}

export default LoginForm
